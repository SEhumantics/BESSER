...
async def export_buml(input_data: DiagramInput):
    ...
    with tempfile.TemporaryDirectory(prefix=f"{TEMP_DIR_PREFIX}{uuid.uuid4().hex}_") as temp_dir:
        ...
        elif elements_data.get("type") == "ActivityDiagram":
            try:
                activity_model = process_activity_diagram(json_data)
            except (KeyError, TypeError, AttributeError) as exc:
                raise ConversionError(f"Malformed Activity diagram payload: {exc}") from exc
            output_file_path = os.path.join(temp_dir, "activity_model.py")
            activity_model_to_code(model=activity_model, file_path=output_file_path)
            file_content = await _read_file(output_file_path, "rb")
            return Response(
                content=file_content,
                media_type="text/plain",
                headers={"Content-Disposition": 'attachment; filename="activity_model.py"'},
            )
...
async def get_single_json_model(buml_file: UploadFile = File(...)):
    ...
    is_activity = any(keyword in content_lower for keyword in [
        'activitymodel(', '.new_initial(', '.new_action(', '.new_decision(', '.new_fork('
    ])
    ...
    if is_project:
        try:
            ...
            for dtype in (
                ...
                "QuantumCircuitDiagram", BPMN_DIAGRAM_TYPE, "ActivityDiagram",
            ):
        ...
        try:
            logger.info("Detected Activity diagram, parsing...")
            activity_json = activity_to_json(buml_content)
            diagram_data = {
                "title": diagram_title,
                "model": activity_json
            }
            ...
            logger.error("Activity diagram parsing failed: %s", str(activity_error))
    ...
    if diagram_data is None or diagram_type is None:
        raise ValueError(
            ...
            "AgentDiagram, GUINoCodeDiagram, NNDiagram, BPMNDiagram, ActivityDiagram, or Project."
        )
